...
pipae3 <- pipae3 |>
  mutate(H=hour(pipae3$Hora),
         ...
         sensor = rep (3, length(pipae3[,1])))


pipae_all=rbind(pipae7, pipae3)


pipaes <- unique(pipae_all$sensor)
ultima_medicao <-  data.frame()
...
for (pipae in pipaes) {
  ...
  diff <- time_length(Sys.Date()-
                      ...
                      unit="day")
  ultima_medicao <- rbind (diff, ultima_medicao)

  }

colnames (ultima_medicao) <-  "ultima medição"
